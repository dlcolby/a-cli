...
def format_session_label(created_at: str, title: str) -> str:
    """Human-readable 'timestamp — title' label for dropdowns/listings. Kept
    separate from the id itself, since the id's timestamp segment must stay
    stable (never regenerated by rename/auto-naming) while the title is free
    to change — this reads the current title, not whatever was baked into
...
def create_session(
    ...
    # No title-derived slug in the id: titles change (auto-naming, /session
    # rename) and the id must not — a slug baked in at creation would either
    # go stale or, for the common case of an untitled session, just be the
    # word "untitled" forever, wasting space for no information.
...
def format_transcript(session: Session) -> str:
    """Color-coded rendering of a session's messages, for reprinting to the
    console when switching into an existing session (so past context is
    visible again, not just silently loaded in the background). Colored so
    user turns stay visually distinguishable from replies when scrolling
    back — this is terminal-display-only, never written to a file."""
